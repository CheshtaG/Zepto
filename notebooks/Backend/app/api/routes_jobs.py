...
async def chat_endpoint(payload: ChatRequest):
  """
  Minimal stub implementation so the frontend chat UI works.
  For now we just echo the last user message and perform no actions.
  """
  last_user = next((m for m in reversed(payload.messages) if m.role == "user"), None)
  content = last_user.content if last_user else "How can I help with your comparison?"
  return ChatResponse(assistant_message=content, actions=[])
